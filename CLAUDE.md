...
- JRC threshold scales **differ between training and inference** because the data sources do: training uses Sen1Floods11's binary JRCWaterHand mask (`dataset.JRC_PERMANENT_THRESHOLD = 0.5`, captures the `1`s), inference uses GEE's "seasonality" band 0–12 (`inference.postprocess`'s `jrc_seasonality_threshold = 5.0`, captures pixels seen as water ≥5 months/yr). These are intentionally different — don't "harmonize" them.
